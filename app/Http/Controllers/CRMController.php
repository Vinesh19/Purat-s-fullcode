...
class CRMController extends Controller
{
    ...
    private function removeSpecificData(Request $request)
    {
        // Validate the input
        $validator = Validator::make($request->all(), [
            'receiver_id' => 'required|string|max:20',
    ...
    private function updateSpecificData(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('agent')) {
                $chatMessages = ChatMessage::where('receiver_id', $receiver_id)->get();

                if ($chatMessages->isNotEmpty()) {
                    foreach ($chatMessages as $chatMessage) {
                        $chatMessage->update(['agent' => $request->agent]);
                    }
                }
            }
